	...
	if current_mode == MODES.NOTHING:
		...
		if current_mode == MODES.FARMING:
			if check_custom_data(mouse_position_to_tile_position, is_hoable_custom_data, farming_layer):
				...
				stack_manager.push4(farming_tile_positions)
				farm_sound.play()
...
func _on_button_4_pressed():
	var tilemap_position = stack_manager.pop4()
	tile_map.set_cells_terrain_connect(farming_layer, tilemap_position, 0, 1)
